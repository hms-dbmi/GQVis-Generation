...
def expand(df, collection):
    ...
    for _, row in df.iterrows():
        logging.info(f"Expanding row with query_template: {row.get('query_template', '')}")
        for dataset_schemas in collection: 
            ...
            for schema in dataset_schemas: # 1 schema is 1 sample ! --> want list of samples. 
                ...
                logging.info(f"Expanding schema: {sample_name}")
            ...
            logging.info(f"Generated {len(new_rows)} expanded rows for schema: {sample_name}")
    ...
    logging.info(f"Total expanded rows: {len(expanded_rows)}")
    return expanded_df
...
def selectGenes(assembly, gene_list, gene_ct=2, chrom_ct=3, popularityWeights=True):
    ...
    df = pd.read_csv(f'/Users/arthe/HMS/copy/DQVis-Generation/location_data/refSeq_chromosomes.hg{assembly_num}.tsv', sep='\t')
    ...
    if len(gene_list) > gene_ct:
        ...
        sampled_genes = []
        df = pd.read_csv(f'/Users/arthe/HMS/copy/DQVis-Generation/location_data/refSeq_genes_scored_compressed.hg{assembly_num}.tsv', sep='\t')
        # subsample dataframe
        subsample = df.sample(
...
def expand_template(row, sample_options, entity_options, field_options, location_options, max_per_template):
    ...
    logging.info(f"Extracted tags: {extract['tags']}")
    ...
    logging.info(f"Constraint solver produced {len(s)} solutions")
    
    #return expand_solutions(row, tags, s)
    
    results = expand_solutions(row, tags, s)
    logging.info(f"Expanded to {len(results)} results for template: {row.get('query_template', '')}")
...
def expand_solutions(row, tags, solutions):
    result = []
    logging.info(f"Expanding solutions for {len(solutions)} solution(s).")
    for s in solutions:
...
if __name__ == "__main__":
    logging.info("Starting multistep expansion script")
    
    with open('/Users/arthe/HMS/copy/DQVis-Generation/data-schema/all-schema.json', 'r') as f:
        schema=json.load(f)
    df=pd.read_csv('multistep_generation2.tsv', sep='\t')

    expanded_df = expand(df, schema)
    logging.info("Expansion complete, writing to CSV")
    print(expanded_df)

    expanded_df.to_csv("multistep_generation_output2.csv")