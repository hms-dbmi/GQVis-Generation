...
from utils import (
    ...
    add_dataframe_columns, create_combined_spec, generate_visual_change_spec
)
#process chains
...
def generate_chain_steps(prev_combined_spec: dict, current_d2_spec: dict, link_template: dict) -> dict:
    ...
    # Always use prev_combined_spec as base and current_d2_spec as the new spec to add
    return create_combined_spec(prev_combined_spec, current_d2_spec, link_template)
...
def export_chains(
        ...
        for chain_instance in range(max_chains_to_generate):
            ...
            for conversation_index in sorted(chain_by_steps.keys()):
                ...
                if chain_instance < len(step_pairs):
                    ...
                    # Verify query continuity (for debugging)
                    if previous_d2_query and conversation_index > 0:
                        d1_query = d1.get('query', d1.get('query_base', ''))
                        print(f"Step {conversation_index} continuity check:")
                        print(f"  Previous D2: {previous_d2_query}")
                        print(f"  Current D1: {d1_query}")
                        # We don't enforce exact matching as templates might be involved
                    ...
                    if conversation_index > 0 and conversation_index - 1 in conversation_specs:
                        ...
                        # For debugging - print the specs being combined
                        print(f"Combining at step {conversation_index}:")
                        print(f"  Prev combined spec keys: {list(prev_combined_spec.keys() if isinstance(prev_combined_spec, dict) else [])}")
                        print(f"  Current D2 spec keys: {list(d2_spec.keys() if isinstance(d2_spec, dict) else [])}")
                        
                        combined_spec = generate_chain_steps(prev_combined_spec, d2_spec, link_template)
                    else:
                        # First step in chain
                        combined_spec = create_combined_spec(d1_spec, d2_spec, link_template)
                    
                    # Store the combined spec for this step
                    conversation_specs[conversation_index] = combined_spec
                    
                    # Update previous D2 query for next iteration
                    previous_d2_query = d2.get('query', d2.get('query_base', ''))
                    
                    dataset_schema = get_dataset_schema(d1, d2, chain_idx)
                    
                    # Create the row for this step
                    row_dict = {
    ...
    # Add a debug column to show the progression of questions in the chain
    if not chains_df.empty and 'D1_query_base' in chains_df.columns and 'D2_query_base' in chains_df.columns:
        chains_df['chain_flow'] = chains_df.apply(
            lambda row: f"Step {row['conversation_index']}: {row['D1_query_base']} -> {row['D2_query_base']}",
            axis=1
        )
    
    chains_df.to_csv("linked_pairs1.csv", index=False)
    print("Chain sequences saved to linked_pairs1.csv")
...
if __name__ == "__main__":
    ...
    print(f"Loaded {len(df)} records from combined dataset")
    print(f"Loaded {len(chains)} chain templates")
    

    max_chains_exported = 100 #change this to change # of results
